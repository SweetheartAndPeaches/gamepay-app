...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE constraint_name = 'fk_payin_orders_user_id'
    AND table_name = 'payin_orders'
  ) THEN
    ALTER TABLE payin_orders 
    ADD CONSTRAINT fk_payin_orders_user_id 
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.table_constraints
    WHERE constraint_name = 'fk_payin_orders_account_id'
    AND table_name = 'payin_orders'
  ) THEN
    ALTER TABLE payin_orders 
    ADD CONSTRAINT fk_payin_orders_account_id 
