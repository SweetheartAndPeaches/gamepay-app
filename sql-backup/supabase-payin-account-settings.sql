...
-- 添加注释
COMMENT ON COLUMN bank_accounts.payin_enabled IS '是否启用该账户进行代收';
COMMENT ON COLUMN bank_accounts.payin_max_amount IS '该账户代收金额上限（0表示无限制）';
COMMENT ON COLUMN bank_accounts.payin_allocated_amount IS '该账户已分配的代收金额';
COMMENT ON COLUMN bank_accounts.payin_earned_commission IS '该账户已获得的佣金总额';
COMMENT ON COLUMN bank_accounts.payin_total_count IS '该账户完成的代收任务总数';
...
-- WHERE user_id = 'xxx' AND payin_enabled = true AND status = 'active';
--
-- 4. 查询账户剩余可分配金额：
-- SELECT
--   id,
